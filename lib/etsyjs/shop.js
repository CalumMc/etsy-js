...
(function() {
  ...
  Shop = (function() {
    ...
    Shop.prototype.findAllShopTransactions = function(_arg, cb) {
      var limit, offset, page, params, secret, token, _ref;
      token = _arg.token, secret = _arg.secret, limit = _arg.limit, offset = _arg.offset, page = _arg.page;
      params = {};
      if (limit != null) {
        params.limit = limit;
      }
      ...
      return (_ref = this.client).get.apply(_ref, ["/shops/" + this.shopId + "/transactions", token, secret].concat(__slice.call(params), [function(err, status, body, headers) {
        if (err) {
          return cb(err);
        }
        if (status !== 200) {
